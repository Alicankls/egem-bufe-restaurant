...
Bu komutlar veritabanı şemasını oluşturur ve mevcut menü verisini (Restaurant + Büfe kategorileri/ürünleri), varsayılan işletme ayarlarını ve `ADMIN_EMAIL`/`ADMIN_PASSWORD` ile bir admin kullanıcısı ekler. `prisma/migrations/` deposu zaten commit'li — kendi `DATABASE_URL`'inize karşı ilk kez çalıştırdığınızda `migrate dev` bu migration'ı uygular, yeni bir tane oluşturmaz.
...
Vercel proje ayarlarında şu environment variable'ları tanımlayın: `DATABASE_URL`, `AUTH_SECRET`, `NEXTAUTH_URL` (production domain), `BLOB_READ_WRITE_TOKEN`. Deploy sonrası, production veritabanına karşı `npx prisma migrate deploy` çalıştırılır (commit'li `prisma/migrations/` dosyalarını uygular).
